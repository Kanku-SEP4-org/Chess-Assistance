...
function IotDashboard() {
  ...
  const fetchIotData = async () => {
    ...
    try {
      ...
      try {
        const lightResponse = await fetch(`${API_URL}/iot/light?id=1`);
        const lightJson = await lightResponse.json();
        if (lightResponse.ok && !lightJson.error) {
          setLightData(lightJson);
        }
      } catch (_) {}

      try {
        const co2Response = await fetch(`${API_URL}/iot/co2?id=1`);
        const co2Json = await co2Response.json();
        if (co2Response.ok && !co2Json.error) {
          setCo2Data(co2Json);
        }
      } catch (_) {}
    } catch (err) {
            ...
            <div>
              <div>
                ...
                <p className="text-white-70 mb-5 fs-5">
                  This dashboard pulls the latest data from the API gateway and
                  the IoT service. It refreshes automatically every 10 seconds.
                </p>
                  ...
                  <strong className="h2 mb-3 d-block">
                    {loading
                      ...
                      : lightData?.value != null
                        ? `${lightData.value.toFixed(1)} lux`
                        : "N/A"}
                  </strong>
                  <p className="text-white-70 mb-0 lh-base">
                    Current reading for the connected light sensor.
                  </p>
                </div>
              </div>
                ...
                <div className="card text-white bg-black bg-opacity-50 border border-white border-opacity-10 rounded-4 p-4 h-100 text-center">
                  ...
                  <strong className="h2 mb-3 d-block">
                    {loading
                      ? "Loading…"
                      : co2Data?.value != null
                        ? `${co2Data.value.toFixed(1)} ppm`
                        : "N/A"}
                  </strong>
                  <p className="text-white-70 mb-0 lh-base">
                    Current reading for the connected CO2 sensor.
                  </p>
                </div>
              </div>
              ...
              <div className="row g-4 align-items-center">
                <div className="col-lg-6">
                  ...
                  <p className="text-white-70 lh-base">
                    ...
                    <strong> {API_URL}</strong> and displays the most recent
                    sensor values.
                  </p>
                </div>
              ...
              <div className="row g-4">
                <div className="col-12">
                  ...
                  <p className="text-white-70 lh-base mb-4">
                    This shows the full response the backend sends to the
                    frontend.
                  </p>
                </div>

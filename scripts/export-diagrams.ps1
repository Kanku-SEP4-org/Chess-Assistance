# ──────────────────────────────────────────────────────────────────────────────
#  export-diagrams.ps1
#  Exports every PlantUML diagram found in .md files to out/<subfolder>/ as PNG
...
foreach ($dir in $SourceDirs) {
    ...
    Get-ChildItem -Path $srcDir -Filter "*.md" -Recurse | ForEach-Object {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "       --> out\$relative\$($file.BaseName).png" -ForegroundColor Green
        } else {
...
if ($count -eq 0) {
    ...
    Write-Host "`nDone - $count diagram(s) exported to out\" -ForegroundColor Green
}

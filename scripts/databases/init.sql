...
CREATE TABLE play_block (
    ...
    ended_at       TIMESTAMP,
    total_duration INTERVAL GENERATED ALWAYS AS (ended_at - started_at),
...
-- Ensure a player can only have one active play block at a time (ended_at IS NULL)
...
CREATE TABLE game (
    ...
    user_rating         INTEGER,
    opp_rating          INTEGER,
    rating_diff         INTEGER GENERATED ALWAYS AS (user_rating - opp_rating) STORED,
    is_player_piece_black BOOLEAN,
    result              game_result_type NOT NULL,
    session_id          INTEGER NOT NULL UNIQUE,
    FOREIGN KEY (session_id) REFERENCES session(id)
